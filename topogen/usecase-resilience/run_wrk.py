...
def kubectl_cp_from_slate_controller_to_host(src_in_pod, dst_in_host):
    ...
    # if temp_file does not exist, create it (just touch)
    run_command(f"touch {temp_file}", required=False)
...
def main():
    ...
    benchmark_name="resilience" # a,b, 1MB and c 2MB file write
    ...
    all_RPS_list = [ \
                    ...
                    ## the same RPS and different network latency
                    ## network latency 1 (oneway): 5ms (10ms RTT)
                    ## network latency 2 (oneway): 50ms (100ms RTT)
                    ## diff: 10x
                    ## capacity threshold for waterfall algorithm and SLATE are equal for 5ms latency
                    ## The same performance is expected in 5ms latency for SLATE and waterfall
                    ## Waterfall will do suboptimal load balancing for 50ms latency by offloading more than it should.
                    ## SLATE will adapt to the network latency and do optimal routing.
                    # {"west":100, "east": 100}, \
                    # {"west":200, "east": 100}, \
                    # {"west":300, "east": 100}, \
                    # {"west":400, "east": 100}, \
    ...
    west_east_lat_list = [0] # west, east, test1
    for we_icl in west_east_lat_list:
        dir_name = f"{dir_name}-lat{we_icl}"
        inter_cluster_latency["node1"]["node2"] = we_icl
