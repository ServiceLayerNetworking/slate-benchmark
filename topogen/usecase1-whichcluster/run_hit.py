...
def update_env_txt(config, routing_rule, rps_dict, inter_cluster_latency, node_to_region):
   ...
   # File to update
    ...
    with open(local_env_file, "w") as file:
        for key, value in config.items():
            if key == "routing_rule":
                file.write(f"{key},{routing_rule}\n")
                print(f"{key},{routing_rule}")
            else:
...
def run_hit(copy_config, routing_rule, target_cluster, target_cluster_rps, wrk_log_path, hit_log_path):
    assert target_cluster_rps >= 0
    if target_cluster_rps == 0:
        msg = f"{target_cluster} {target_cluster_rps} RPS is skipped"
        print(msg)
        return msg
    ...
    print(f'cluster_rps: {target_cluster_rps}')
...
def main():
    ...
    CONFIG = {
        ...
        'duration': 60,
        'cpu_noise': 70,
        'benchmark_name': "usecase1-whichcluster",
    ...
    for rps_dict in all_RPS_list:
        ...
        for routing_rule in CONFIG['routing_rule']:
            ...
            hit_log_path_dict = dict()
            for cluster in rps_dict:
                wrk_log_path_dict[cluster] = f"{output_dir}/{routing_rule}-{CONFIG['mode']}-c{CONFIG['cpu_noise']}-d{CONFIG['degree']}-R{rps_dict[cluster]}-{cluster}.wrklog"
                hit_log_path_dict[cluster] = f"{output_dir}/{routing_rule}-{CONFIG['mode']}-c{CONFIG['cpu_noise']}-d{CONFIG['degree']}-R{rps_dict[cluster]}-{cluster}.hitlog"
            def recalculate_capacity(capacity, rps_dict):
                ...
                if total_demand > total_capacity:
                    ...
                    return capacity
            CONFIG['capacity'] = recalculate_capacity(CONFIG['capacity'], rps_dict)
            print(f"** recalculated_capacity: {CONFIG['capacity']}")
            ''' update env.txt and scp to slate-controller pod '''
            local_env_file = update_env_txt(CONFIG, routing_rule, rps_dict, inter_cluster_latency, node_to_region)
            kubectl_cp_from_host_to_slate_controller_pod(local_env_file, "/app/env.txt")
            if CONFIG['mode'] == "runtime":
                slatelog = f"{CONFIG['benchmark_name']}-trace.csv"
            ...
            with concurrent.futures.ThreadPoolExecutor() as executor:
                for cluster in wrk_log_path_dict:
                    print(f"cluster: {cluster}, RPS: {rps_dict[cluster]}")
                ...
                sleep_before_resource_usage_recording = CONFIG['duration'] - 10 # 60 -> 50s
                ...
                for cluster in wrk_log_path_dict:
                    ...
                    print(f"record resource usage for {cluster}")
                
                final_sleep = CONFIG['duration']-sleep_before_resource_usage_recording
                assert final_sleep > 0
                print(f"final sleep for {final_sleep} seconds")
                time.sleep(final_sleep)
                print("final sleep done")
                # ''' Join all threads '''
                # for future in concurrent.futures.as_completed(future_list):
                #     print(future.result())
                
                ''' merge hit log into wrklog '''
                for cluster in hit_log_path_dict:
                    run_command(f"cat {hit_log_path_dict[cluster]} >> {wrk_log_path_dict[cluster]}")
            ...
            if CONFIG['mode'] == "profile":
                ...
                print("???")
                print(f"mode: {CONFIG['mode']} is not supported")
                exit()
