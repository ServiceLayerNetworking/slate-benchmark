...
def run_wrk(copy_config, target_cluster, req_type, target_cluster_rps, wrk_log_path):
    ...
    if target_cluster_rps >= 500:
        copy_config["connection"] = target_cluster_rps-200
    if target_cluster_rps >= 500:
    ...
    wrk_command = f'./wrk -D {copy_config["distribution"]} -t{copy_config["thread"]} -c{copy_config["connection"]} -d{copy_config["duration"]} -r -L -S -s ./{target_cluster}_{req_type}.lua {server_ip} -R{target_cluster_rps} | grep -v "Thread calibration" >> {wrk_log_path}'
...
def main():
    ...
    ########################################################
    benchmark_name="usecase1-orderofevent" # a,b, 1MB and c 2MB file write
    ...
    mode_and_routing_rule = {\
        ...
        # "runtime": ["LOCAL", "SLATE", "MCLB",  "WATERFALL2", "REMOTE"],\
    }
    ...
    all_RPS_list = [ \
                    ...
                    ## usecase1-orderofevent
                    {"west": 300, "south": 300, "central": 50, "east": 50}, \
                    ]
    ...
    node_dict = get_nodename_and_ipaddr("/users/gangmuk/projects/slate-benchmark/config.xml")
    
    def network_latency_for_order_of_event(mode, routing_rule, node_dict, node_to_region):
        ...
        if routing_rule == "SLATE":
            ...
            inter_cluster_latency["node3"]["node4"] = 50 # central, south
            inter_cluster_latency["node1"]["node4"] = 200 # west, south
            inter_cluster_latency["node2"]["node3"] = 200 # east, central
            inter_cluster_latency["node1"]["node3"] = 5 # west, central ** for slate
            inter_cluster_latency["node2"]["node4"] = 5 # east, south ** for slate
        else:
            inter_cluster_latency["node1"]["node2"] = 51 # west, east
            inter_cluster_latency["node3"]["node4"] = 50 # central, south
            inter_cluster_latency["node1"]["node4"] = 200 # west, south
            inter_cluster_latency["node2"]["node3"] = 200 # east, central
            inter_cluster_latency["node1"]["node3"] = 200 # west, central ** for waterfall
            inter_cluster_latency["node2"]["node4"] = 200 # east, south ** for waterfall
    ...
    for mode in mode_and_routing_rule:
        for rps_dict in all_RPS_list:
            ...
            print(f"* postfix for output file: {dir_postfix}")
            for req_type in req_type_list:
                for routing_rule in mode_and_routing_rule[mode]:
                    
                    inter_cluster_latency = network_latency_for_order_of_event(mode, routing_rule, node_dict, node_to_region)
                    
                    per_wrk_st = datetime.now()
                    ''' Initialize the output path for different logs'''
                    output_dir = f"{dir_postfix}/"
