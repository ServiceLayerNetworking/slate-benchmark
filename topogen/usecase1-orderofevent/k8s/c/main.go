...
func main() {
	...
	r.POST("/spreadc", POSTspreadc)

	r.Run(":8080")
}

func POSTspreadc(c *gin.Context) {
	var data []byte
	RunCPULoad(0, 0)
	// WriteToFile("/dev/null", 0)
	// WriteToFile("/dev/null", 2097152)
	...
	rand.Read(data)
	client = &http.Client{}
	req, err = http.NewRequest("POST", "http://d:8080/spreadd", bytes.NewBuffer(data))
	if err != nil {
		panic(err)
	}
